// @leetcode id=1914 questionId=2043 slug=cyclically-rotating-a-grid lang=cpp site=leetcode.com title="Cyclically Rotating a Grid"
class Solution {
public:
    vector<vector<int>> rotateGrid(vector<vector<int>>& grid, int k) {
        int m = grid.size(), n = grid[0].size();
        int layers = min(m, n) / 2;
        
        for (int layer = 0; layer < layers; layer++) {
            // Extract layer elements in order (counter-clockwise traversal)
            vector<int> elems;
            int top = layer, bottom = m - 1 - layer;
            int left = layer, right = n - 1 - layer;
            
            // Top row: left to right
            for (int col = left; col <= right; col++)
                elems.push_back(grid[top][col]);
            // Right col: top+1 to bottom
            for (int row = top + 1; row <= bottom; row++)
                elems.push_back(grid[row][right]);
            // Bottom row: right-1 to left
            for (int col = right - 1; col >= left; col--)
                elems.push_back(grid[bottom][col]);
            // Left col: bottom-1 to top+1
            for (int row = bottom - 1; row >= top + 1; row--)
                elems.push_back(grid[row][left]);
            
            int sz = elems.size();
            int rot = k % sz;
            // Rotate left by rot (counter-clockwise shift)
            vector<int> rotated(elems.begin() + rot, elems.end());
            rotated.insert(rotated.end(), elems.begin(), elems.begin() + rot);
            
            // Put back
            int idx = 0;
            for (int col = left; col <= right; col++)
                grid[top][col] = rotated[idx++];
            for (int row = top + 1; row <= bottom; row++)
                grid[row][right] = rotated[idx++];
            for (int col = right - 1; col >= left; col--)
                grid[bottom][col] = rotated[idx++];
            for (int row = bottom - 1; row >= top + 1; row--)
                grid[row][left] = rotated[idx++];
        }
        
        return grid;
    }
};
