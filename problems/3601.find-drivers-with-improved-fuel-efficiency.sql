-- @leetcode id=3601 questionId=3942 slug=find-drivers-with-improved-fuel-efficiency lang=mysql site=leetcode.com title="Find Drivers with Improved Fuel Efficiency"
# Write your MySQL query statement below
WITH first_half AS (
    SELECT driver_id, AVG(distance_km / fuel_consumed) AS avg_eff
    FROM trips
    WHERE MONTH(trip_date) BETWEEN 1 AND 6
    GROUP BY driver_id
),
second_half AS (
    SELECT driver_id, AVG(distance_km / fuel_consumed) AS avg_eff
    FROM trips
    WHERE MONTH(trip_date) BETWEEN 7 AND 12
    GROUP BY driver_id
)
SELECT
    d.driver_id,
    d.driver_name,
    ROUND(f.avg_eff, 2) AS first_half_avg,
    ROUND(s.avg_eff, 2) AS second_half_avg,
    ROUND(s.avg_eff - f.avg_eff, 2) AS efficiency_improvement
FROM drivers d
JOIN first_half f ON d.driver_id = f.driver_id
JOIN second_half s ON d.driver_id = s.driver_id
WHERE s.avg_eff > f.avg_eff
ORDER BY efficiency_improvement DESC, d.driver_name ASC;
